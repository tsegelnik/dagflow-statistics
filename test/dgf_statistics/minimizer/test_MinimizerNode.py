...
from numpy import allclose, fromiter, linspace
...
from dgf_statistics.minimizer.MinimizerNode import MinimizerNode
...
def test_MinimizerNode(mu, sigma, mode, testname):
    size = 201
    ...
    with Graph(close=True) as graph:
        ...
        minimizer = MinimizerNode("Minimizer")
        chi >> minimizer("statistic")
        MuFit >> minimizer
        SigmaFit >> minimizer
    ...
    assert allclose(res["covariance"], minimizer._minimizer.calculate_covariance(), rtol=0, atol=1e-8)
...
def draw_params(res, mu, sigma, minimizer, figname):
    _, ax = plt.subplots()
    for cl in (1, 2, 3):
        contours = minimizer._minimizer._minimizer.mncontour(0, 1, cl=cl)
